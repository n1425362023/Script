...
try:
    font = ImageFont.truetype("simfang.ttf", font_size)  # 可根据实际路径修改字体
except IOError:
...
def save_code_as_image(file_path, code_imgs_path):
    ...
    y_text = 10
    for line in lines:
        draw.text((10, y_text), line, font=font, fill=(255, 255, 255))  # 白色文字
...
def save_output_as_image(output_text, effect_imgs_path, script_path, user_inputs=None, prompt_texts=None):
    lines = []
    ...
    if prompt_texts is not None:
        for prompt_text, user_input in zip(prompt_texts, user_inputs):
...
def get_input_prompt(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        match = re.findall(r'input\(\s*["\'](.*?)["\']\s*\)', content)
        if match:
            # 遍历所有捕获组
            prompt = match
    return prompt
...
# 函数：运行脚本并捕获输入和输出
def run_script_with_input(script_path, user_inputs):
    ...
    # 如果你需要将 stdout 或 stderr 的字节串解码回字符串
    # 捕获输出并组合标准输出和标准错误
    output_text = stdout + '\n' + stderr
    return output_text
...
for script in os.listdir(programs_dir):
    if script.endswith('.py'):
        ...
        if input_prompt:
            print(input_prompt)
            for i, group in enumerate(input_prompt, start=1):
                user_inputs.append(input(f"{script_name}" + "---" + f"group {i}" + "---" + f"{group} "))
        # 4. 运行 Python 脚本并捕获输入和输出
        print(f"Running {script_path} with input")
        output_text = run_script_with_input(script_path, user_inputs)
        ...
        save_output_as_image(output_text, output_image_file, script_path, user_inputs, input_prompt)
